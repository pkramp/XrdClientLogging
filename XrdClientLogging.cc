...
public:
  ...
  ClientLoggingFile(std::string x, std::string loggingPath)
      : XrdCl::File(false), loggingPath(loggingPath), writtenSize(0),
        readSize(0) {}

  // Destructor
  ~ClientLoggingFile() {
    // Create the file to log in. We use a LockedFile so many Clients can log to
    // one file in parallel.
    LockedFile lFile(loggingPath + "/XrdClientLogging.log");
    std::stringstream ss;
    std::string lastUrl;
    XrdCl::File::GetProperty("LastURL", lastUrl);
    ss << (readSize ? "r" : "w") << "," << lastUrl.substr(0, 7) << "," << path
       << "," << (readSize ? readSize : writtenSize) << "," << startTime << ","
       << stopTime << "," << stopTime - startTime << "," << XrdVERSION << "\n";
  ...
  virtual XRootDStatus Open(const std::string &url, OpenFlags::Flags flags,
    ...
    startTime =
        duration_cast<milliseconds>(system_clock::now().time_since_epoch())
            .count();
    return XrdCl::File::Open(url, flags, mode, handler, timeout);
  }
  ...
  virtual XRootDStatus Read(uint64_t offset, uint32_t length, void *buffer,
    ...
    readSize += length;
    return XrdCl::File::Read(offset, length, buffer, handler,
                             timeout); // call base
  }
  ...
  virtual XRootDStatus Write(uint64_t offset, uint32_t size, const void *buffer,
    ...
    writtenSize += size;
    return XrdCl::File::Write(offset, size, buffer, handler,
                              timeout); // call base
  }

  virtual bool IsOpen() const { return XrdCl::File::IsOpen(); }
